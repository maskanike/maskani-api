...
module.exports = {
   ...
   * @param {object} notificationMetaData - metadata used to send notification
   */
  async sendReceiptEmail(user, tenant, notificationMetaData) {
    const subject = `Your Receipt for Invoice ${notificationMetaData.InvoiceId} | ${notificationMetaData.flat}`

    const htmlMessage =
      `Thank you for being a tenant at <b> ${notificationMetaData.flat} </b> unit  <b> ${notificationMetaData.unit}</b>. ` +
      `Your payment for Invoice ${notificationMetaData.InvoiceId} of amount <b> ${notificationMetaData.amount}  KSHS</b> has been received.</p>`
