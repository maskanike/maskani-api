module.exports = {
  ...
  async down(queryInterface) {
    const transaction = await queryInterface.sequelize.transaction()
    try {
      await queryInterface.removeColumn('Invoices', 'outdated', {
        transaction
      })
      await queryInterface.removeColumn('Receipts', 'outdated', {
        transaction
      })
      await queryInterface.removeColumn('Statements', 'outdated', {
        transaction
      })
      await transaction.commit()
    } catch (err) {
